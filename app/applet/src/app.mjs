import{TEST_PROFILES,CONNECTION_MODES,splitTransferBytes,calculateMbps,median,latencyJitter,probeFailPercent,parseProviderMeta,videoSuitability,isCompleteResult,speedFraction,formatMiB,mergeHistoryRecords}from"./measurement.mjs";
import{networkHealthIndex,healthBand,useCaseSuitability,throughputStats,diagnosticFlags}from"./quality.mjs";
...
async function measureDownload(id){
  ...
  let lastPaint=started,lastSampleAt=started,lastSampleBytes=0;
  ...
  try{
    ...
    const mbps=calculateMbps(received,performance.now()-started);
    ...
    return{mbps,stats:throughputStats(chartSamples)};
  }catch(error){
    scope.abort();
  ...
  return new Promise((resolve,reject)=>{
    ...
    const finish=()=>{
      ...
      try{
        ...
        const mbps=calculateMbps(sent,performance.now()-started);
...
function userError(error){const message=String(error?.message||"");if(message===t("timeout"))return message;if(/latency/i.test(message))return t("latencyError");if(/download/i.test(message))return t("downloadError");if(/upload/i.test(message))return t("uploadError");return t("genericError")}async function startOrStop(){if(running){controller?.abort();abortCurrentUploads();return}running=true;runId++;const id=runId;controller=new AbortController();setRunning(true);resetForRun();try{const meta=await loadMetadata(id);setProgress(8);const latency=await measureLatency(id),downloadResult=await measureDownload(id),download=downloadResult.mbps,upload=await measureUpload(id);ensureRun(id);setProgress(100);setPhase("complete");ui.transfer.textContent=t("complete");const result={completed:true,aborted:false,downloadMbps:download,uploadMbps:upload,latencyMs:latency.latency,jitterMs:latency.jitter,probeFailPct:latency.failPct,throughputVariationPct:downloadResult.stats?.variationPct??null,throughputMinMbps:downloadResult.stats?.minMbps??null,throughputMaxMbps:downloadResult.stats?.maxMbps??null,throughputSampleCount:downloadResult.stats?.sampleCount??0,profile:profileId,connectionMode:connectionModeId,streamCount:CONNECTION_MODES[connectionModeId].streams,edge:meta?.edge||"--",timestamp:new Date().toISOString()};if(isCompleteResult(result)){history.push(result);history=history.slice(-100);lastResult=result;localStorage.setItem(HISTORY_KEY,JSON.stringify(history));renderHistory();renderVideo();renderHealth()}}catch(e){if(e?.name==="AbortError"){setPhase("stopped");ui.transfer.textContent=t("stopped")}else{const message=userError(e);setPhase("error");ui.transfer.textContent=message;toast(message)}}finally{if(id===runId){setRunning(false);controller=null;currentXhrs.clear()}}}
...
function renderHistory(){const list=$("historyList"),empty=$("historyEmpty");list.innerHTML="";empty.textContent=t("historyEmpty");empty.hidden=history.length>0;[...history].reverse().forEach(r=>{const item=document.createElement("article");item.className="history-item";item.innerHTML=`<div class="history-main"><strong>↓ ${r.downloadMbps.toFixed(1)} / ↑ ${r.uploadMbps.toFixed(1)} Mbps</strong><span>${new Date(r.timestamp).toLocaleString(lang==="th"?"th-TH":"en-US")} • ${t(r.profile||"standard")} • ${t((r.connectionMode||"single")==="multi"?"multiConnection":"singleConnection")}</span></div><div class="history-meta">${r.latencyMs.toFixed(1)} ms<br>${r.edge||"--"}</div>`;list.appendChild(item)})}
...
$("shareButton").addEventListener("click",shareLatest);$("exportButton").addEventListener("click",()=>{if(!history.length){toast(t("noResult"));return}const blob=new Blob([JSON.stringify(history,null,2)],{type:"application/json"}),url=URL.createObjectURL(blob),a=document.createElement("a");a.href=url;a.download=`zipspeed_history_${Date.now()}.json`;a.click();URL.revokeObjectURL(url)});$("clearButton").addEventListener("click",()=>{history=[];lastResult=null;localStorage.removeItem(HISTORY_KEY);renderHistory();renderVideo();renderHealth();toast(t("cleared"))});
