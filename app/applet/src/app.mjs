...
async function measureDownload(id){
  ...
  try{
    ...
    return{mbps,stats:throughputStats(chartSamples),loadedLatencyMs:loadedSamples.length?median(loadedSamples):null,loadedLatencySampleCount:loadedSamples.length};
  }catch(error){
    scope.abort();
    if(controller?.signal.aborted)throw new DOMException("Aborted","AbortError");
...
function renderHealth(result=lastResult){
  ...
  const loadedLatency=Number(result?.loadedLatencyMs);$("loadedLatencyValue").textContent=Number.isFinite(loadedLatency)?`${loadedLatency.toFixed(1)} ms`:"--";
}
