...
import{networkHealthIndex,healthBand,useCaseSuitability,throughputStats,diagnosticFlags,compareResults}from"./quality.mjs";
...
function renderHistory(){const list=$("historyList"),empty=$("historyEmpty");list.innerHTML="";empty.textContent=t("historyEmpty");empty.hidden=history.length>0;renderHistoryComparison();[...history].reverse().forEach(r=>{const item=document.createElement("article");item.className="history-item";item.innerHTML=`<div class="history-main"><strong>↓ ${r.downloadMbps.toFixed(1)} / ↑ ${r.uploadMbps.toFixed(1)} Mbps</strong><span>${new Date(r.timestamp).toLocaleString(lang==="th"?"th-TH":"en-US")} • ${t(r.profile||"standard")} • ${t((r.connectionMode||"single")==="multi"?"multiConnection":"singleConnection")}</span></div><div class="history-meta">${r.latencyMs.toFixed(1)} ms<br>${r.edge||"--"}</div>`;list.appendChild(item)})}
