...
function measureUpload(id){
  ...
  return new Promise((resolve,reject)=>{
    const sentByStream=new Array(chunks.length).fill(0),started=performance.now(),loadedProbePromises=[],probeTimers=[];
    let completed=0,settled=false,finishing=false;
    ...
    const finish=async()=>{
      ...
      try{
        ...
        resolve({mbps,sentBytes:sent,durationMs,loadedLatencyMs:loadedSamples.length?median(loadedSamples):null,loadedLatencySampleCount:loadedSamples.length});
