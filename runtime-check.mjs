...
try{
 ...
 if(!app.includes("runTest")||!app.includes("measureUnderLoad")||!app.includes("/cdn-cgi/trace")||!app.includes("window.zipspeedStopForLifecycle"))throw new Error("Runtime interaction/metadata/lifecycle code missing");
 ...
 if(process.env.ZIPSPEED_REAL_NETWORK_SMOKE==="1"){const target=enabled[0].baseUrl.replace(/\/$/,"");const [down,trace]=await Promise.all([fetch(target+"/__down?bytes=1024",{cache:"no-store"}),fetch(target+"/cdn-cgi/trace",{cache:"no-store"})]);if(!down.ok)throw new Error("Real-network download endpoint HTTP "+down.status);if(!trace.ok)throw new Error("Real-network trace endpoint HTTP "+trace.status);const b=await down.arrayBuffer(),raw=await trace.text();if(b.byteLength<1024)throw new Error("Real-network endpoint returned insufficient bytes");const fields=Object.fromEntries(raw.trim().split(/\r?\n/).map(line=>{const i=line.indexOf("=");return i>0?[line.slice(0,i),line.slice(i+1)]:[line,""]}));evidence.realNetworkSmoke=true;evidence.realNetworkBytes=b.byteLength;evidence.reportedEdge=fields.colo||null;evidence.reportedCountry=fields.loc||null}
